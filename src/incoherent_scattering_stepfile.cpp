...
int main(int argc, char* argv[]) {
    std::string Directory{}, ParameterFile {}, MoleculeFile{}, ConfigFile{}, ConfigFileStart{}, WQFile {}, IncoherentDir{}, IncoherentFileName{}, IncoherentFileNameStart{}, StepFile{};
    int StartStep{}, Step{}, SamplingStep{}, T0Step {}, T1Step{};
    ...
    if (argc != 8) {
        std::cout << "usage: ./Incoherent_scattering RUNPARENTDIR PARAMETERFILE WQFILE STARTSTEP STEPFILE SAMPLINGSTEP DELTAT" << std::endl;  
        return EXIT_FAILURE; 
    }    
    ...
    std::cout << "using the following q: " << std::endl; 
    for (auto& vec : q) std::cout << vec << std::endl; 
    ...
    while (true) {
        ...
        while (StepVectorIterator != StepVector.end()) {
	        ...
	        for (unsigned k = 0; k < q.size(); k++){
	            std::cout << wq_current[k]/NumberOfMonomers << std::endl; 
